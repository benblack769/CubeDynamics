...
CubeChangeInfo CubeInfo::get_bordering_quantity_vel(const CubeInfo & other_cube,glm::vec3 cube_direction,float solid_interal_bond)const{
    ...
    */
    ...
    float amt_air_given = std::min(basic_air_amt * this->data.air_mass * seconds_per_calc, this->data.air_mass/(0.0001f+float(SIDES_ON_CUBE)));
    float amt_liquid_given = std::min(basic_liquid_amt * this->data.liquid_mass * seconds_per_calc,this->data.liquid_mass/(0.0001f+float(SIDES_ON_CUBE)));
    float amt_solid_given = std::min(basic_solid_amt * this->data.solid_mass * seconds_per_calc,this->data.solid_mass/(0.0001f+float(SIDES_ON_CUBE)));
